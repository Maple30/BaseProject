...
$(document).ready(function () {
	...
	//HS觀光景點
	$('.HS1').click(function HS1() {
		$title.stop().text('臺北市');
		$p1.stop().text('原住民族群：平埔族');
		$p2.stop().text('臺北市是臺灣最大的都市。也是政治、經濟、文化的中心。臺北的整體發展相當活潑多元，充滿朝氣。');
		// $p2.stop().append("<p class='morelink' style='color:#007bff'><a href='#'>平埔族</a> 相關介紹</p>");
	});

	$('.HS2').click(function HS2() {
		$title.stop().text('基隆市');
		$p1.stop().text('原住民族群：阿美族');
		$p2.stop().text('基隆位於臺灣北部，三面環山，僅北面一處有少量平原迎向大海，即為深水谷灣之基隆港，為進入北臺灣門戶。');
		$p2.stop().append("<p class='morelink' style='color:#007bff'><a href='{% url 'Amis' %}'>阿美族</a> 相關介紹</p>");
	});
	$('.HS3').click(function () {
		$title.stop().text('新北市');
	...
	$('.HS5').click(function () {
		...
		$p2.stop().append("<p class='morelink' style='color:#007bff'><a href='{% url 'Tayal' %}'>泰雅族</a> <a href='{% url 'SaySiyat' %}'>賽夏族</a> 相關介紹</p>");		
	});
	$('.HS6').click(function () {
		$title.stop().text('新竹市');
		$p1.stop().text('原住民族群：阿美族');
	...
	$('.HS7').click(function () {
		...
		$p2.stop().append("<p class='morelink' style='color:#007bff'><a href='{% url 'Tayal' %}'>泰雅族</a> <a href='{% url 'SaySiyat' %}'>賽夏族</a> 相關介紹</p>");
		// $p2.stop().append("<p class='morelink' style='color:#007bff'><a href='#'>泰雅族</a> <a href='#'>賽夏族</a> <a href='#'>平埔族</a> 相關介紹</p>");
	});
	$('.HS8').click(function () {
		$title.stop().text('臺中市');
		$p1.stop().text('原住民族群：阿美族 泰雅族');
	...
	$('.HS9').click(function () {
		$title.stop().text('彰化縣');
		$p1.stop().text('原住民族群：無');
		$p2.stop().text('彰化素有「臺灣穀倉」之稱，東南北三面環山，西臨臺灣海峽，境內以八卦山為主峰。彰化古稱「半線」。');
	});
	$('.HS10').click(function () {
		$title.stop().text('雲林縣');
		$p1.stop().text('原住民族群：無');
		$p2.stop().text('雲林縣位於臺灣的中南部，嘉南平原的最北端，全縣幾乎是沖積平原，是臺灣的榖倉及農業縣，縣內農產品眾多。');
	});
	...
	$('.HS11').click(function () {
		...
		$p2.stop().append("<p class='morelink' style='color:#007bff'><a href='{% url 'Seediq' %}'>賽德克族</a> <a href='{% url 'Bunun' %}'>布農族</a> <a href='{% url 'Thao' %}'>邵族</a> <a href='{% url 'Tayal' %}'>泰雅族</a> <a href='{% url 'Tsou' %}'>鄒族</a> 相關介紹</p>");
	});
	$('.HS12').click(function () {
		$title.stop().text('嘉義縣');
		$p1.stop().text('原住民族群：鄒族');
		$p2.stop().text('地處嘉南平原，為重要農業產區。境內水利建設堪稱完備，土地肥沃，以農產聞名。');
		$p2.stop().append("<p class='morelink' style='color:#007bff'><a href='{% url 'Tsou' %}'>鄒族</a> 相關介紹</p>");		
	});
	$('.HS14').click(function () {
		$title.stop().text('嘉義市');
		$p1.stop().text('原住民族群：無');
		$p2.stop().text('位於臺灣西南部「嘉南平原」北端，北回歸線經過本市南邊約一公里處。');
	});
	$('.HS13').click(function () {
		$title.stop().text('臺南市');
		$p1.stop().text('原住民族群：平埔族');
		$p2.stop().text('臺南市是臺灣的發祥地，是全臺歷史最悠久的都市。臺南市在17世紀以前，是臺灣平地原住民西拉雅族的生活領域，所謂的「西拉雅四大社」皆位於此。');
	});
	$('.HS15').click(function () {
		$title.stop().text('高雄市');
		...
		$p2.stop().append("<p class='morelink' style='color:#007bff'><a href='{% url 'Bunun' %}'>布農族</a> <a href='{% url 'Paiwan' %}'>排灣族</a> <a href='{% url 'Rukai' %}'>魯凱族</a> <a href='{% url 'Saaroa' %}'>拉阿魯哇族</a> 相關介紹</p>");		
	});
	$('.HS16').click(function () {
		$title.stop().text('屏東縣');
		...
		$p2.stop().append("<p class='morelink' style='color:#007bff'><a href='{% url 'Amis' %}'>阿美族</a> <a href='{% url 'Paiwan' %}'>排灣族</a> <a href='{% url 'Rukai' %}'>魯凱族</a> 相關介紹</p>");		
	});
	$('.HS17').click(function () {
		$title.stop().text('宜蘭縣 ');
		$p1.stop().text('原住民族群：泰雅族');
		$p2.stop().text('宜蘭市位居蘭陽平原中心的精華地帶，河川縱橫提供豐富的灌溉資源，讓居於平原中央的宜蘭市擁有肥沃土地與充裕水源，成為一個農產豐饒的稻米之鄉。');
		$p2.stop().append("<p class='morelink' style='color:#007bff'><a href='{% url 'Tayal' %}'>泰雅族</a> 相關介紹</p>");		
	});
	$('.HS18').click(function () {
		$title.stop().text('花蓮縣');
		...
		$p2.stop().append("<p class='morelink' style='color:#007bff'><a href='{% url 'Taroko' %}'>太魯閣族</a> <a href='{% url 'Sakizaya' %}'>撒奇萊雅族</a> <a href='{% url 'Seediq' %}'>賽德克族</a> <a href='{% url 'Bunun' %}'>布農族</a> <a href='{% url 'Amis' %}'>阿美族</a> <a href='{% url 'Kavalan' %}'>噶瑪蘭族</a> 相關介紹</p>");		
	});
	$('.HS20').click(function () {
		$title.stop().text('連江縣');
		$p1.stop().text('原住民族群：無');
		$p2.stop().text('馬祖列島以「媽祖」而得名，位於臺灣海峽西北西方，隸屬於福建省連江縣，面臨閩江口、連江口及羅源灣，為目前台澎金馬自由地區最北端之領土。');
	});
	$('.HS19').click(function () {
		$title.stop().text('臺東縣');
		...
		$p2.stop().append("<p class='morelink' style='color:#007bff'><a href='{% url 'Bunun' %}'>布農族</a> <a href='{% url 'Pinuyumayan' %}'>卑南族</a> <a href='{% url 'Amis' %}'>阿美族</a> <a href='{% url 'Paiwan' %}'>排灣族</a> <a href='{% url 'Yami' %}'>達悟族</a> <a href='{% url 'Rukai' %}'>魯凱族</a> 相關介紹</p>");		
	});
	$('.HS21').click(function () {
		$title.stop().text('金門縣');
		$p1.stop().text('原住民族群：無');
		$p2.stop().text('金門，一個曾經在歷史上因為屢次勝利戰役而赫赫有名的自由聖地，是中華民國政府所轄的島縣。');
	});
	$('.HS22').click(function () {
		$title.stop().text('澎湖縣');
		$p1.stop().text('原住民族群：無');
		$p2.stop().text('澎湖舊稱「平湖」、「澎瀛」、「澎海」，又稱「西瀛」、「島夷」等等，澎湖的建築多與福建、金門等地相似。');
	});
});
